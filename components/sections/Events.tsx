      ...
      <div className="grid grid-cols-3 gap-0.5 max-lg:grid-cols-1">
        {events.map((ev, i) => (
          <RevealOnScroll key={ev.id} delay={Math.min(i + 1, 4) as 1 | 2 | 3 | 4}>
            <div
              className={`ecard bg-[var(--ink2)] border border-[var(--border-g)] p-8.5 relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-[rgba(76,175,98,0.35)] ${
                ev.featured
            ...
            >
              {/* Tag */}
              <span
                className={`inline-block font-mono text-[9px] tracking-[0.2em] uppercase px-2.5 py-1 border mb-5 ${
              ...
              {/* Arrow */}
              <div className="absolute bottom-6.5 right-6.5 w-9 h-9 border border-[var(--border-g)] flex items-center justify-center text-[var(--green-lt)] transition-all duration-300 group-hover:bg-[var(--navy)] group-hover:border-[var(--navy)] group-hover:text-white group-hover:rotate-45">
                →
              </div>
            </div>
          </RevealOnScroll>
