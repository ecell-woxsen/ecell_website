        ...
        <div className="grid grid-cols-4 gap-3 max-lg:grid-cols-2 max-sm:grid-cols-1">
          {initiatives.map((init, i) => (
            <RevealOnScroll key={init.id} delay={Math.min(i + 1, 4) as 1 | 2 | 3 | 4}>
              <div
                className="ipillar bg-[var(--paper2)] border border-[rgba(8,13,28,0.08)] px-6.5 py-9.5 rounded-2xl transition-all duration-300 relative overflow-hidden hover:bg-[var(--navy-deep)] hover:-translate-y-1.5 group"
                data-n={init.number}
              >
                <div className="w-12 h-12 border border-[rgba(8,13,28,0.14)] rounded-xl flex items-center justify-center text-xl mb-7 transition-all duration-300 group-hover:bg-[var(--green)] group-hover:border-[var(--green)]">
                  {init.icon}
                </div>
