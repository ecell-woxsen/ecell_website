...
void main() {
  ...
  // 2D simplex noise flow spreading aurora ribbons smoothly across the entire vertical section
  float n1 = snoise(vec2(uv.x * 1.8 + uTime * 0.08, uv.y * 1.2 + uTime * 0.12)) * 0.5 * uAmplitude;
  float n2 = snoise(vec2(uv.x * 1.2 - uTime * 0.06, uv.y * 1.8 + uTime * 0.09)) * 0.4 * uAmplitude;
  float intensity = 0.45 + n1 + n2;
  
  float auroraAlpha = smoothstep(0.05 - uBlend * 0.3, 0.7 + uBlend * 0.3, intensity);
